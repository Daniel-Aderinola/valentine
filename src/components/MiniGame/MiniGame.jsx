...
const createDeck = () =>
  shuffle([...EMOJI_PAIRS, ...EMOJI_PAIRS].map((emoji, index) => ({
    id: index,
    emoji,
    flipped: false,
    matched: false,
...
const MiniGame = () => {
  ...
  const getWinMessage = () => {
    if (moves <= 12) return "Perfect memory! Just like how I'll never forget the day I fell for you 💕";
    if (moves <= 18) return "Amazing! You matched all the love — just like we're a perfect match! 🥰";
    if (moves <= 25) return "You found every pair! That's how easy it is to find love when it's real 💖";
    ...
    <section className={styles['game-section']} id="game">
      ...
      {gameState === 'idle' && (
        <div className={styles['start-screen']}>
          ...
          <h3>Ready to Play?</h3>
          <p>
            Match all the romantic pairs to unlock a special message!
            How few moves can you do it in? 😉
          </p>
      ...
      {gameState === 'playing' && (
        <>
          <div className={styles['game-stats']}>
            ...
            <span className={styles.stat}>✨ Pairs: {matches}/{EMOJI_PAIRS.length}</span>
